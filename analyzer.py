...
[등급 분류 세부 기준]
- S (초대형): 삼성/LG/현대차 등 대기업 대상 제3자 배정 유상증자, 지분 매각(M&A), 세계 최초 기술 상용화. 상한가 안착 가능성 90% 이상.
- A (강력): 대규모 공급계약(전년 매출 30% 이상), 정부 정책 핵심 수혜, 신사업 진출 확정. 시초가 갭 15~20% 형성 가능.
...
2. Reasoning: 재료가 해당 종목의 시가총액 대비 얼마나 큰 돈이 되는가? (시총 대비 계약규모 등 고려)
3. Result: 위 기준에 따른 등급(S~C), 섹터, 투자 포인트 작성.
...
- reason: 왜 이 등급을 부여했는지 판단 근거 요약
- reference_url: 원문 링크 (반드시 제공된 데이터 내의 링크 사용)

[주의사항]
- 뻔한 뉴스에 S를 주지 마라. (남발 시 봇의 신뢰도 하락)
